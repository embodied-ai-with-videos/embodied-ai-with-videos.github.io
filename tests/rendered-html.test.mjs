...
test("server-renders the complete course outline", async () => {
  ...
  assert.match(html, /N pen-and-paper quizzes/);
  assert.doesNotMatch(html, /random pen-and-paper quizzes/);
...
test("publishes the complete Lecture 1 deck through the local viewer", async () => {
  const [viewerHtml, parts] = await Promise.all([
    readFile(new URL("../lecture1.html", import.meta.url), "utf8"),
    readdir(new URL("../lecture1-parts-b64/", import.meta.url)),
  ]);

  assert.match(viewerHtml, /Lecture 1 slides/);
  assert.match(viewerHtml, /new Blob\(buffers, \{ type: "application\/pdf" \}\)/);
  assert.match(viewerHtml, /atob\(encoded\)/);
  assert.equal(parts.filter((name) => /^lecture1\.part\.\d{2}\.b64$/.test(name)).length, 20);
});

test("publishes the complete Lecture 3 deck through the local viewer", async () => {
  const [viewerHtml, parts] = await Promise.all([
    readFile(new URL("../lecture3.html", import.meta.url), "utf8"),
    readdir(new URL("../lecture3-parts-b64/", import.meta.url)),
  ]);

  assert.match(viewerHtml, /Lecture 3 slides/);
  assert.match(viewerHtml, /new Blob\(buffers, \{ type: "application\/pdf" \}\)/);
  assert.match(viewerHtml, /atob\(encoded\)/);
