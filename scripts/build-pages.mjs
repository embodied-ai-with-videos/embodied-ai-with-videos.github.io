...
const rootLecture1PartsDir = join(root, "lecture1-parts-b64");
const rootLecture3PartsDir = join(root, "lecture3-parts-b64");
...
for (const filename of [
  ...
  "lecture7.html",
  "og.png",
  "university.shield.rgb.blue.png",
]) {
  await cp(join(pagesDir, filename), join(root, filename));
}

await rm(rootLecture1PartsDir, { force: true, recursive: true });
await rm(join(root, "lecture1-parts"), { force: true, recursive: true });
await cp(join(pagesDir, "lecture1-parts-b64"), rootLecture1PartsDir, { recursive: true });
